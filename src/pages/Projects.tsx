...
const projects: Project[] = [
  {
    ...
    links: { github: "https://github.com/d3v07/Aegis-X" },
  },
  {
    number: "02",
    title: "CivicProof",
    ...
    links: { github: "https://github.com/d3v07/civicproof" },
  },
  {
    number: "03",
    title: "CIMphony",
    ...
    links: { github: "https://github.com/d3v07/cimphony" },
  },
  {
    number: "04",
    title: "Metropolis",
    ...
    links: { github: "https://github.com/d3v07/GeminiHackathon" },
  },
  {
    number: "05",
    title: "DocWeave",
  ...
  },
  {
    ...
    links: { github: "https://github.com/d3v07/predictr" },
  },
  {
    number: "12",
    title: "SpendLens",
  ...
  },
  {
    ...
    links: { github: "https://github.com/d3v07/Axiom" },
  },
  {
    number: "15",
  ...
  },
  {
    ...
    links: { github: "https://github.com/d3v07/NEXACORE" },
  },
  {
    number: "17",
    title: "RideShare",
  ...
  },
  {
    ...
    title: "Sift",
    filename: "Sift.py",
    category: "AI / Database Reliability",
    domain: "Infrastructure",
    tagline: "Evidence-driven MongoDB reliability agent with hash-bound human approval and verified index fixes",
    problem: "Slow MongoDB queries from missing or mis-ordered indexes are risky to fix: the Equality-Sort-Range key order is fiddly, AI tools apply changes without review, and there's rarely proof the fix worked.",
    solution: "Sift captures real query workloads with explain evidence, uses Gemini-powered Agent Engine roles to propose ESR-correct indexes, then requires a hash-bound human approval before a deterministic controller applies and re-verifies the change.",
    impact: ["Docs examined collapsed from 100,377 to 15 after a verified index fix, with the blocking sort removed — measured by re-running explain, not an LLM claim", "Hash-bound approval gate: no index is created without a human reviewing the exact evidence", "Every phase (detect, diagnose, approve, apply, verify) written to an audit ledger", "Voyage AI retrieval surfaces runbook context without influencing the deterministic decision path"],
    features: ["Hash-bound human approval where deterministic Python computes the evidence hash", "Read-only Vertex AI Agent Engine roles that narrate but never mutate", "Strict verify rail: a fix is VERIFIED only if the blocking sort is removed and a metric improves", "MongoDB MCP server for read-only explain access; Voyage AI for runbook retrieval"],
    tech: ["Python", "FastAPI", "Next.js", "Gemini", "Vertex AI Agent Engine", "MongoDB Atlas", "MCP", "Voyage AI", "Cloud Run"],
    links: { github: "https://github.com/d3v07/googlecloudhack", live: "https://gcrah-dashboard-2vbnam7yma-uc.a.run.app" },
  },
  {
    number: "19",
    title: "Concord",
    filename: "Concord.py",
    category: "AI / Multi-Agent Observability",
    domain: "AI",
    tagline: "Contract-to-repair diagnostic layer that detects, attributes, and patches AG2 multi-agent violations",
    problem: "Multi-agent systems fail silently — an agent claims it verified sources but records zero, another runs a side-effect action without approval — and there's no tooling to catch, root-cause, and repair these contract violations.",
    solution: "Concord sits outside the target workflow, reads its execution trace, and runs a seven-stage pipeline: deterministic contract checks, LLM attribution and patch generation, Daytona-sandboxed regression testing, and a human approval gate — without touching the monitored workflow.",
    impact: ["288 tests (274 unit + 14 integration-marked) covering the diagnostic pipeline", "Detects 3 intentional contract violations in the demo fixture and maps each to the correct AG2 primitive", "Repair confidence scored deterministically from primitive mapping, without an LLM", "Daytona sandbox runs LLM-generated regression tests in isolation, deleting state even on failure"],
    features: ["Two-zone design: a read-only diagnostic pipeline that never touches the monitored agents", "Deterministic contract checker applies a rule set against a folded context snapshot before any LLM call", "Primitive-map repair step picks the correct AG2 fix target (Guardrail, HumanGate, Handoff) without an LLM", "Daytona sandbox executes a self-contained regression script and returns pass/fail per violation"],
    tech: ["Python", "AG2", "Gemini 2.5 Flash", "OpenRouter", "Tavily", "Daytona", "FastAPI", "React"],
    links: { github: "https://github.com/d3v07/AG2_Hackathon", live: "https://concord-lite.vercel.app/" },
  },
  {
    number: "20",
    title: "Unsyphn",
    ...
    domain: "AI",
    tagline: "Always-on autonomous agent that monitors SaaS vendor terms, diffs every change, and routes findings",
    problem: "Enterprise SaaS contracts drift constantly — vendors edit DPAs, add sub-processors in non-adequate jurisdictions, and raise per-seat prices — but nobody notices until renewal day.",
    solution: "Unsyphn runs a seven-stage pipeline every 60 seconds per vendor — headless fetch, Gemini structured-output diff, ClickHouse snapshot storage, policy rule evaluation, and routing to Slack, Jira, Email, Calendar, and Senso — with every state change pushed over a single SSE event bus.",
    impact: ["84 tests across 16 files passing, with typecheck and build clean", "Six external tools wired as typed providers in the autonomous loop, not curl wrappers", "Stripe webhook closes the loop: verifies signature, flips org entitlements, and fires an SSE event", "Append-only change-report store keeps a full version history so the audit log is the diff between versions"],
    features: ["No run button: a scheduler ticks every 60 seconds and fans out the pipeline per vendor", "Gemini structured-output diff validated through Zod, with a heuristic classifier fallback", "Change-report state machine (new, acknowledged, snoozed, resolved) with typed transitions", "Branded TypeScript IDs so cross-org leaks become compile errors"],
    tech: ["TypeScript", "Node.js", "Hono", "React", "Zod", "ClickHouse", "Gemini 2.5 Pro", "Stripe", "Vitest"],
    links: { github: "https://github.com/d3v07/Datadog_Hackathon" },
  },
  {
    number: "21",
    title: "Mean It",
    ...
    problem: "AI writing tools jump straight to generation, producing polished text with no traceable connection to what the user meant. The output feels borrowed, not authored.",
    solution: "Mean It runs a structured interview first — choosing a moment, a guide persona, and concrete details — then assembles a final artifact where every line links back to the exact question and answer that produced it, shown as a shareable provenance trace.",
    impact: ["254 tests passing across the full interview-to-provenance pipeline", "Provenance matcher links every output line to the specific user answer that grounded it", "Byline meter tracks authorship percentage and cannot be gamed by guide configuration", "Full export flow: poster download, caption options, and a shareable trace link"],
    features: ["Multi-step interview engine with interchangeable guide personas", "Spine validator that lifts verbatim user phrases into a three-movement structure", "Provenance matcher with hover tooltips showing the source question and answer per line", "Audio transcription, TTS, and video curation helpers for multimodal intake"],
    tech: ["Next.js", "React", "TypeScript", "Tailwind CSS", "Anthropic SDK", "OpenAI Whisper", "Vitest"],
    links: { github: "https://github.com/d3v07/cbc" },
  },
  {
    number: "22",
    title: "Mod Vanguard",
    filename: "ModVanguard.ts",
    category: "Platform Tooling / Moderation",
    domain: "Engineering",
    tagline: "Reddit-native moderation command center that turns scattered report bursts into structured, owned casefiles",
    problem: "Subreddit mod teams lose context across report queues, mod notes, appeal threads, and prior decisions spread across surfaces, forcing every ruling to start from memory and leaving audit history unrecoverable.",
    solution: "Built as a Devvit Web app installed directly on Reddit, Mod Vanguard converts reports, menu actions, and appeal events into structured casefiles with ownership, searchable decision memory, appeal linking, rule-drift analytics, and scoped audit export — while keeping every final action human-controlled.",
    impact: ["Twelve distinct moderator workflow surfaces shipped, from dashboard and casefile to appeals desk and audit trail", "Decision memory makes prior rule-tagged rulings searchable so moderators reuse precedent", "Signal intake surface proves whether report triggers and scheduled jobs are arriving, catching silent failures", "Scoped JSON and CSV audit export for accountability and transparency"],
    features: ["Report radar clusters repeated reports into pressure tiers linked to one reviewable casefile", "Claim-and-handoff system with expiry timers prevents duplicate moderator work", "Rule ops aggregates cases, decisions, and appeals by rule to surface drift over time", "Devvit-native integration using Redis-backed state and moderator-scoped server routes"],
    tech: ["TypeScript", "Devvit", "Reddit Developer Platform", "Redis", "pnpm"],
    links: { github: "https://github.com/d3v07/modcase_reddit", live: "https://developers.reddit.com/apps/mod-vanguard" },
  },
  {
    number: "23",
    title: "Newark IT Store",
    filename: "NewarkIT.py",
    category: "Database / Retail Management",
    domain: "Engineering",
    tagline: "Full-stack retail management system over a normalized MySQL schema with CRUD, order workflow, and reporting",
    problem: "Demonstrating end-to-end relational database skills requires a realistic schema with referential integrity, multi-table joins, and reporting queries — not a single-table CRUD toy.",
    solution: "Built a Flask app backed by a normalized MySQL schema covering customers, addresses, stored cards, baskets, products, offers, and transactions, with an order-placement workflow, date-range statistics reports, and a Docker Compose setup that seeds data for immediate browser verification.",
    impact: ["50 seeded customers, 11 products, and 50 transactions verified live in the browser after Docker Compose startup", "Full order lifecycle: basket creation, item management, offer pricing, and transaction recording", "Six sales statistics report types covering top customers, product performance, and basket analytics", "Credential hygiene fixed: database password moved from source code to environment variables"],
    features: ["Customer, address, and stored-card management with full edit and history views", "Basket workflow with per-item quantity editing and offer-price calculation", "Transaction history with filters and date-range statistics reports", "Docker Compose with schema and seed scripts for one-command local setup"],
    tech: ["Python", "Flask", "MySQL", "Jinja2", "Docker Compose"],
    links: { github: "https://github.com/d3v07/NEWARK_IT_APP" },
  },
  {
    number: "24",
    title: "AutoChain eSupplyPro",
    filename: "eSupplyPro.ts",
    category: "AI / Supply Chain Platform",
    domain: "AI",
    ...
    solution: "Built a TypeScript monorepo with Next.js and Fastify delivering three tailored portals — customer, vendor, and admin — each with a persistent AI assistant supporting text, voice, visual, and agentic interaction modes for workflow automation.",
    impact: ["Three fully functional role-based portals (customer, vendor, admin) with distinct workflows", "AI assistant supporting four interaction modes, including autonomous workflow execution", "39 tests across 9 files covering API routes, auth, workflows, and assistant sessions", "Seeded with realistic data: 28 orders, 19 invoices, 17 shipments, and 12 assistant sessions"],
    features: ["Persistent AI assistant with text, voice, visual, and agentic modes across all portals", "Approval-based workflow automation with an autonomous execution option", "EDI monitoring for purchase orders, invoices, and shipping transactions", "Prompt-driven chart generation and AI-generated versioned documents"],
    tech: ["Next.js", "Fastify", "TypeScript", "SQLite", "Drizzle ORM", "Ollama", "Turborepo", "Vitest"],
    links: { github: "https://github.com/d3v07/autochain" },
  },
  {
    number: "25",
    title: "NetSentinel",
    filename: "NetSentinel.py",
    ...
    problem: "Identifying anomalous network flows without labeled training data requires an unsupervised approach that scores traffic in real time and surfaces which features drove the flag.",
    solution: "Trained an IsolationForest over 23 CICIDS2017-style flow features on a bundled synthetic dataset — no external download required — and exposed real scoring, alerts, and per-flow feature explanations through a JWT-authenticated REST API backed by a Vite React dashboard.",
    impact: ["Anomaly scoring pipeline runs end-to-end out of the box with no dataset download", "Returns an anomaly score, severity, and top contributing features per flow", "Backend pytest suite covers model and API paths", "Honest README distinguishes working components from roadmap stubs — no fabricated accuracy claims"],
    features: ["IsolationForest over 23 flow features with per-flow feature-deviation explanations", "JWT-authenticated REST API with analyze, alerts, and stats endpoints", "Optional LSTM autoencoder and SHAP explainability via separate requirements", "LangChain policy-suggestion agent skeleton (requires an OpenAI key)"],
    tech: ["Python", "Flask", "scikit-learn", "IsolationForest", "React", "Vite", "JWT", "Docker"],
    links: { github: "https://github.com/d3v07/NetSentinel.ai", live: "https://frontend-beige-psi-18.vercel.app" },
  },
  {
    number: "26",
    title: "RoomRadar",
    filename: "RoomRadar.ts",
    category: "AI / Productivity",
    domain: "AI",
    tagline: "Paste an event URL, get a goal-ranked plan of who to meet, why, and how to open",
    problem: "Walking into a networking event without context means wasted time — most people leave without knowing who the highest-leverage person in the room was or how to approach them.",
    solution: "RoomRadar scrapes any event URL via Firecrawl, enriches every speaker and sponsor with public signal, and produces a goal-aware ranked action plan with contextual conversation openers, a room strategy score, and a shareable brief.",
    impact: ["Five goal modes (collaborators, hiring, investment, operators, partnerships) that re-weight the entire ranking", "Generates per-person talk tracks, a best first move, and follow-up email templates from one URL paste", "Runs fully in demo mode with no API key via a bundled sample", "Stateless by design: no accounts, no persistence, no tracking"],
    features: ["Goal-aware scoring engine ranks attendees by fit across five networking objectives", "Firecrawl-powered structured extraction of speakers, sponsors, schedule, and branding", "Deep-scan and feature-blueprint generators surface high-priority moves", "Shareable print-friendly brief with timeline, talk tracks, and follow-up templates"],
    tech: ["TanStack Start", "React", "Vite", "TypeScript", "Firecrawl", "Tailwind CSS", "TanStack Query", "Zod"],
    links: { github: "https://github.com/d3v07/roomradarr" },
  },
  {
    number: "27",
    title: "HFT Lab",
    ...
    problem: "Building a credible trading terminal prototype requires handling real-time order-book streaming, message-contract validation, and order lifecycle without a broker dependency.",
    solution: "Designed as a TypeScript monorepo with a WebSocket backend that normalizes Kraken and mock feeds into a shared Zod-validated message bus, an in-memory limit-order book for paper fills, and a terminal-style React UI driven by Zustand.",
    impact: ["67 tests passing across backend and shared-schema suites", "End-to-end order lifecycle from submission through ACK, fill, and position update verified in mock mode", "Multi-source routing: Kraken WebSocket for crypto, Yahoo Finance polling for equities, mock generator offline", "Full-workspace TypeScript and Vite production builds verified clean"],
    features: ["Real-time WebSocket order-book streaming with Kraken gateway normalization", "In-memory toy matching engine with ACK, fill, cancel, and reject message flow", "Shared Zod schemas enforcing message contracts across frontend, backend, and tests", "React terminal UI with a live order book, positions panel, and audio alerts"],
    tech: ["Node.js", "TypeScript", "React", "Vite", "Zustand", "Zod", "Express", "WebSocket", "MongoDB", "Vitest"],
    links: { github: "https://github.com/d3v07/HFT" },
  },
  {
    number: "28",
    title: "HealthMesh",
    filename: "HealthMesh.tsx",
    ...
    problem: "The operational surface between providers, payers, and pharma teams is fragmented across incompatible data standards (FHIR, HL7, X12) with no unified plane for prior auth, claims review, and outcomes analytics.",
    solution: "Built a full-stack monorepo with React role dashboards, a JWT-authenticated Express API with audit middleware, dual MongoDB and PostgreSQL storage, Socket.IO updates, and Python service boundaries exposing FHIR, HL7, and X12 adapters alongside ML stubs.",
    impact: ["Four distinct role dashboards (provider, payer, pharma, admin), each surfacing domain-specific workflows", "Audit middleware attached to every API route for compliance-ready event logging", "Interoperability adapters spanning FHIR, HL7, and X12 in a single integration surface", "Docker Compose brings up MongoDB and PostgreSQL with seed scripts in one command"],
    features: ["Role-based React dashboards for provider, payer, pharma, and admin personas", "JWT auth and RBAC with per-route audit logging middleware", "Python interop adapters for FHIR, HL7, and X12 healthcare data standards", "ML service stubs for prior-auth copilot, claims anomaly scoring, and outcomes prediction"],
    tech: ["React", "TypeScript", "Node.js", "Express", "MongoDB", "PostgreSQL", "Socket.IO", "FastAPI", "Docker"],
    links: { github: "https://github.com/d3v07/Healthmesh" },
  },
  {
    number: "29",
    title: "Strake",
    ...
    problem: "Container terminal operators need to see future yard states — crane paths, vehicle routes, container positions — before they happen, but pixel-reconstruction AI is too slow for real-time port planning.",
    solution: "Built a three-layer system: sensor and TOS feeds are encoded into a latent space by a JEPA-based world model, next-state vectors are predicted and reduced to 3D coordinates, then streamed over WebSocket to a React Three Fiber canvas that renders predicted splines over a WebGL yard.",
    impact: ["Sub-second planning cycles at 0.98s vs 47s for pixel-reconstruction approaches (per the repo's own benchmark)", "No stop-gradients, no EMA teacher, no pixel reconstruction — predictions stay in latent space", "GPU inference offloaded to serverless RunPod/Modal containers, keeping the API server CPU-only", "Spline rendering with selective bloom for real-time spatial visualization"],
    features: ["JEPA encoder trained with sketched isotropic Gaussian regularization", "PCA/UMAP reduction from high-dimensional latent vectors to renderable 3D coordinates", "FastAPI WebSocket bridge streaming latent predictions to the Three.js frontend", "Glassmorphic HUD overlays and scroll-driven camera navigation over a WebGL port yard"],
    tech: ["Next.js", "TypeScript", "React Three Fiber", "Three.js", "FastAPI", "PyTorch", "scikit-learn", "RunPod", "Modal"],
    links: { github: "https://github.com/d3v07/strake" },
  },
];
